...
<body>
	...
	<div class = "container">
		...
		<section>
			<article class = "order">
				<h2>주문 정보 수정</h2>
				<form name = "form" method = "post" action = "update_orderProcess.jsp?p_orderno=<%=p_orderno %>">
				<%
				sql = "select b.point, a.address, a.request, a.p_code, a.order_qty from p_order a, m_member b where a.id = b.id and p_orderno = ?";
				int p_price = 0;
				try {
					...
					if(rs.next()) {
						...
						p_code = rs.getString(4);
						String order_qty = rs.getString(5);
						%>
						<div class = "name">
						...
						<div class = "request">
							<input  style = "width : 50%;"type = "text" name = "request" id = "request" readOnly value = "<%=req %>">
							<select name = "req" onchange = "req_change()">
								<option value = "0" <%=req.equals("0") ? "selected" : "" %>>선택하세요</option>
								<option value = "9" <%=req.equals("9") ? "selected" : "" %>>직접 입력</option>
								<option value = "집 앞에 두고 문자주세요." <%=req.equals("집 앞에 두고 문자주세요.") ? "selected" : "" %>>집 앞에 두고 문자주세요.</option>
								<option value = "경비실에 맡겨 주세요." <%=req.equals("경비실에 맡겨 주세요.") ? "selected" : "" %>>경비실에 맡겨 주세요.</option>
								<option value = "오실 때 전화 주세요." <%=req.equals("오실 때 전화 주세요.") ? "selected" : "" %>>오실 때 전화 주세요.</option>
							</select>
						...
						<div class = "total">
							<input type = "text" name = "total" id = "total" readOnly value = "<%=p_price * Integer.parseInt(order_qty) %>">
